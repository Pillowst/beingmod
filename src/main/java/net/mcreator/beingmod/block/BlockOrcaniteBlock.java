...
public class BlockOrcaniteBlock extends BeingmodModElements.ModElement {
	@ObjectHolder("beingmod:block_orcanite")
	public static final Block block = null;
	public BlockOrcaniteBlock(BeingmodModElements instance) {
		super(instance, 2);
	}
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("block_orcanite", "block_orcanite", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == Blocks.STONE.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 39)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(32, 0, 0, 50))));
		}
	}
}
